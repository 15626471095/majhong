...
function host:attach(sp)
	return function(name, args, session)
		...
		if args then
			...
			return core.pack(header ..  content), proto.response~=nil
		else
			return core.pack(header), proto.response~=nil
		end
	end
end

package.loaded["common.sproto.sproto"] = sproto;
